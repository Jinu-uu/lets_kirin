...
export class TakenCoursesService {
    ...
    private async updateUserSkills(userId: string, courseName: string): Promise<void> {
        try {
            ...
            let currentSkills = typeof user.skillLevels === 'string' 
            ...
            skillMapping.skills.forEach(skill => {
                if (currentSkills.hasOwnProperty(skill.type)) {
                    currentSkills[skill.type] = Number(currentSkills[skill.type]) + Number(skill.points);
                } else {
    ...
    async uploadExcel(file: Express.Multer.File, userId: string): Promise<UploadResponseDto> {
        try {
            // 스킬 레벨 초기화
